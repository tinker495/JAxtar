...
from qfunction.neuralq.hlg_qlearning import (
    get_hlg_qlearning_dataset_builder,
    hlg_qlearning_builder,
)
...
def hlg_qlearning(
    puzzle: Puzzle,
    puzzle_opts: PuzzleOptions,
    qfunction: HLGNeuralQFunctionBase,
    puzzle_name: str,
    train_options: DistTrainOptions,
    shuffle_length: int,
    with_policy: bool,
    ...
    **kwargs,
    ...
    print_config("Q-Learning Training Configuration", config)
    logger = create_logger(train_options.logger, f"{puzzle_name}-dist-hlg-q-train", config)
    key = jax.random.PRNGKey(
        np.random.randint(0, 1000000) if train_options.key == 0 else train_options.key
    )
    key, subkey = jax.random.split(key)
    ...
    qlearning_fn = hlg_qlearning_builder(
        train_options.train_minibatch_size,
        qfunc_model,
        optimizer,
        qfunction.pre_process,
    ...
    get_datasets = get_hlg_qlearning_dataset_builder(
        puzzle,
        qfunction.pre_process,
        qfunc_model,
        train_options.dataset_batch_size,
    ...
    for i in pbar:
        ...
        if i % 100 == 0:
            logger.log_histogram("Losses/Diff", diffs, i)
            logger.log_histogram("Losses/Softmax Cross Entropy", sces, i)
            logger.log_histogram("Metrics/Target", target_q, i)
    ...
    if eval_options.num_eval > 0:
        ...
        _run_evaluation_sweep(
            ...
            puzzle_name=puzzle_name,
            search_model=qfunction,
            search_model_name="hlg_qfunction",
            search_builder_fn=qstar_builder,
            eval_options=eval_options,
            puzzle_opts=puzzle_opts,
            output_dir=eval_run_dir,
