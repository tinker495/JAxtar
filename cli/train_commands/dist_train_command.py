...
def davi(
    ...
    **kwargs,
    ...
    get_datasets = get_heuristic_dataset_builder(
        ...
        warmup_steps=train_options.diffusion_distance_warmup_steps,
    ...
    for i in pbar:
        key, subkey = jax.random.split(key)
        dataset = get_datasets(target_heuristic_params, eval_params, subkey, step=i)
...
def qlearning(
    ...
    **kwargs,
    ...
    for i in pbar:
        key, subkey = jax.random.split(key)
        dataset = get_datasets(target_qfunc_params, eval_params, subkey, step=i)
