...
def replay_train_option(func: callable) -> callable:
    ...
    @click.option("-ab", "--add_batch_size", type=int, default=128)
...
def dist_zeroshot_qfunction_options(func: callable) -> callable:
    @wraps(func)
    def wrapper(*args, **kwargs):
        puzzle_bundle = kwargs.pop("puzzle_bundle")
        ...
        zeroshot_q_callable = puzzle_bundle.zeroshot_q_function_nn
        if zeroshot_q_callable is None:
            raise click.UsageError(f"Zeroshot Q-function not available for puzzle '{puzzle_name}'.")

        zeroshot_qfunction: ZeroshotQFunctionBase = zeroshot_q_callable(puzzle_size, puzzle, reset)
