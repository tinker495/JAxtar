...
class MoEResMLPModel(DistanceModel):
    Res_N: int = 4
    ...
    def _forward(self, x, training=False, capture_aux=False):
    ...
    def __call__(self, x, training=False):
        x, _ = self._forward(x, training=training, capture_aux=False)
        return x

    def train_loss(self, x, target, actions=None, loss_type="mse", loss_args=None, **kwargs):
        pred, moe_aux = self._forward(x, training=True, capture_aux=True)
