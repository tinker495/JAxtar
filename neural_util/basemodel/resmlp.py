...
class SelfPredictiveResMLPModel(SelfPredictiveDistanceModel):
    ...
    def setup(self):
        ...
        if self.use_swiglu:
            ...
            self.initial_norm = FunctionalNorm(norm_fn=self.norm_fn, dtype=DTYPE)
            self.second_dense = nn.Dense(self.hidden_dim, dtype=DTYPE)
            if self.resblock_fn != PreActivationResBlock:
                self.second_norm = FunctionalNorm(norm_fn=self.norm_fn, dtype=DTYPE)
        ...
        if self.resblock_fn == PreActivationResBlock:
            self.final_norm = FunctionalNorm(norm_fn=self.norm_fn, dtype=HEAD_DTYPE)
