...
class SPRQModel(nn.Module):
    ...
    def __call__(self, x, training=False):
        ...
        projected_p = self.projection_head(latent_z, training)  # (batch_size, latent_dim)
        transition = self.transition_model(
            latent_z, training
        )  # (batch_size, action_size, latent_dim)
        transition = transition[:, 0]  # (batch_size, latent_dim)

        predicted_next_p = self.predicton_head(transition, training)  # (batch_size, latent_dim)
