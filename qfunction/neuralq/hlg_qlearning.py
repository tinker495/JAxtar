...
from qfunction.neuralq.hlg_neuralq_base import HLGNeuralQFunctionBase, HLGQModelBase
...
def hlg_qlearning_builder(
    ...
    def qlearning_loss(
        ...
        loss = jnp.mean(sce * weights)
        return loss, (new_params, sce)
    ...
    def qlearning(
        ...
        def train_loop(carry, batched_dataset):
            ...
            (loss, (q_params, sce)), grads = jax.value_and_grad(qlearning_loss, has_aux=True)(
                q_params,
                solveconfigs,
                states,
                actions,
            ...
            return (q_params, opt_state), (loss, grad_magnitude_mean, sce)

        (q_params, opt_state), (losses, grad_magnitude_means, sces) = jax.lax.scan(
            train_loop,
            (q_params, opt_state),
            (
    ...
    if n_devices > 1:
        ...
        def pmap_qlearning(key, dataset, q_params, opt_state):
            ...
            (qfunc_params, opt_state, loss, grad_magnitude, weight_magnitude, sces) = jax.pmap(
            ...
            sces = jnp.concatenate(sces)
            return qfunc_params, opt_state, loss, grad_magnitude, weight_magnitude, sces
...
def _get_datasets_with_policy(
    puzzle: Puzzle,
    preproc_fn: Callable,
    q_model: HLGNeuralQFunctionBase,
    minibatch_size: int,
    target_q_params: Any,
    q_params: Any,
    ...
    def get_minibatched_datasets(key, vals):
        ...
        (_, q_values), _ = q_model.apply(q_params, preproc, training=False, mutable=["batch_stats"])
...
def get_hlg_qlearning_dataset_builder(
    puzzle: Puzzle,
    preproc_fn: Callable,
    q_model: HLGQModelBase,
    dataset_size: int,
    shuffle_length: int,
