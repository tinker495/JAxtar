...
class SokobanNeuralQ(NeuralQFunctionBase):
    ...
    def solve_config_pre_process(self, solve_config: Sokoban.SolveConfig) -> chex.Array:
        target_board = self.puzzle.unpack_board(solve_config.TargetState.board)
        one_hot_board = jax.nn.one_hot(target_board, num_classes=4)
        flattened_board = jnp.reshape(one_hot_board, (-1,))
        return (flattened_board - 0.5) * 2.0

    def state_pre_process(self, state: Sokoban.State) -> chex.Array:
