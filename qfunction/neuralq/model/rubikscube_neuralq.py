...
class RubiksCubeTransformerQModel(nn.Module):
    action_size: int
    num_classes: int
    seq_len: int
    embed_dim: int = 128
    ...
    def __call__(self, tokens: chex.Array, training: bool = False) -> chex.Array:
        ...
        head = nn.relu(head)
        head = nn.Dense(self.action_size, dtype=HEAD_DTYPE)(head)
        return head


class _BaseRubiksCubeTransformerQ(NeuralQFunctionBase):
    def __init__(
        self,
        puzzle: RubiksCube,
        *,
        ...
        **kwargs,
        ...
        super().__init__(
            puzzle,
            model=RubiksCubeTransformerQModel,
...
class RubiksCubeNeuralQ(_BaseRubiksCubeTransformerQ):
    def __init__(self, puzzle: RubiksCube, **kwargs):
        super().__init__(puzzle, include_target=False, **kwargs)


class RubiksCubeRandomNeuralQ(_BaseRubiksCubeTransformerQ):
    def __init__(self, puzzle: RubiksCube, **kwargs):
