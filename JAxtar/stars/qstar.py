...
def qstar_builder(
    ...
    def qstar(
        ...
        def _body(input: tuple[SearchResult, jnp.ndarray, Puzzle.State, HashIdx, chex.Array]):
            ...
            if use_q_fn_params and q_fn_params is not None:
                q_vals = q_fn.batched_q_value(solve_config, states, params=q_fn_params)
            else:
    ...
    if use_q_fn_params:
        params = q_fn.get_params()
        if params is None:
            qstar_fn(empty_solve_config, empty_states)
        else:
            qstar_fn(empty_solve_config, empty_states, params)
    else:
        qstar_fn(empty_solve_config, empty_states)
