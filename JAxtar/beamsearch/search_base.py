...
def beam_loop_continue_if_not_solved(
    search_result: BeamSearchResult,
    puzzle: Puzzle,
    solve_config: Puzzle.SolveConfig,
) -> chex.Array:
    """Shared loop continuation check for beam variants."""
...
def finalize_beam_search_result(
    search_result: BeamSearchResult,
    puzzle: Puzzle,
    solve_config: Puzzle.SolveConfig,
) -> BeamSearchResult:
    """Populate solved metadata for the final beam state."""
    filled_mask = search_result.filled_mask()
    ...
    solved_idx = jnp.where(solved_any, solved_idx.astype(jnp.int32), jnp.array(-1, dtype=jnp.int32))
...
def apply_selected_candidates(
    ...
    """Write selected candidates into beam slots and trace buffers."""
