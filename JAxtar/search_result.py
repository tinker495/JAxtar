...
def pop_full(qstar_result: SearchResult):
    ...
    min_key, min_val, qstar_result.min_key_buffer, qstar_result.min_val_buffer = merge_sort_split(
        min_key, min_val, qstar_result.min_key_buffer, qstar_result.min_val_buffer
    )
    ...
    def _body(val):
        ...
        qstar_result.priority_queue, min_key_buffer, min_val_buffer = BGPQ.delete_mins(
            qstar_result.priority_queue
        )
        min_key_buffer = jnp.where(
            qstar_result.not_closed[min_val_buffer.index, min_val_buffer.table_index],
            min_key_buffer,
            jnp.inf,
        )
        (
            min_key,
            min_val,
            qstar_result.min_key_buffer,
            qstar_result.min_val_buffer,
        ) = merge_sort_split(min_key, min_val, min_key_buffer, min_val_buffer)
        filled = jnp.isfinite(min_key)
        return qstar_result, min_key, min_val, filled

    qstar_result, min_key, min_val, filled = jax.lax.while_loop(
        _cond, _body, (qstar_result, min_key, min_val, filled)
    )
